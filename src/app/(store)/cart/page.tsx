          ...
          <div className="flex-1">
            ...
            <div className="divide-y divide-border">
              {cartItems.map((item, index) => (
                <div key={index} className="py-6 flex flex-col sm:grid sm:grid-cols-12 gap-4 items-start sm:items-center">
                  
                  {/* Product Info */}
                  <div className="col-span-6 flex gap-4 w-full">
                    <Link href={`/products/${item.product.slug}`} className="w-20 sm:w-24 aspect-[3/4] bg-secondary shrink-0 relative block">
                      <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${item.product.images[0]}')` }} />
                    </Link>
                    <div className="flex flex-col pt-1">
                      <Link href={`/products/${item.product.slug}`} className="font-medium hover:underline underline-offset-4 mb-1 text-sm sm:text-base">
                        {item.product.name}
                      </Link>
                      ...
                      <button className="text-xs text-muted-foreground underline underline-offset-2 mt-auto self-start sm:hidden pt-4">
                        Remove
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="col-span-3 flex justify-start sm:justify-center w-full mt-4 sm:mt-0">
                    <div className="flex items-center border border-input w-28 h-9">
                      <button className="flex-1 flex justify-center items-center text-muted-foreground hover:text-foreground">
                  ...
                  <div className="col-span-3 flex justify-between sm:justify-end items-center w-full mt-4 sm:mt-0 hidden sm:flex">
                    ...
                    <button className="ml-4 text-muted-foreground hover:text-foreground p-2">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
